 ...
 *
 * 为什么会有问题呢?//我们在写入数据以后，修改了java文件(实际上是class文件)，再读的时候发现。报错了。
 ...
 * 		wirte数据： oos.txt -- id=100
 * 		read数据: oos.txt -- id=100
 * ！！！实际上说的就是：因为你改了class文件。所以class文件对应的id值就发生了变化。而你写的数据是没有变化的。所以id值就会不同。
 ...
 * 
 * 注意：
 * 		我一个类中可能有很多的成员变量，有些我不想进行序列化(值为默认值，不会被赋值而改变)。请问该怎么办呢?
